<%@ page contentType="text/html;charset=UTF-8" language="java" %>
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
		<% if (session.getAttribute("user")==null) { response.sendRedirect("/work-ai/login.jsp"); return; } String
			success=request.getParameter("success"); if (request.getAttribute("workOrders")==null) {
			...
			<body>
				...
				<c:if test="${param.success == 'true'}">
					<div id="success" class="alert alert-success alert-dismissible fade show" role="alert">
						...
						<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"
							onClick="hideTab('success')"></button>
					</div>
				</c:if>
								...
								<ul class="nav nav-tabs" id="workOrderTabs" role="tablist">
									<li class="nav-item" role="presentation">
										<button class="nav-link active" id="create-tab" data-bs-toggle="tab"
											...
											aria-selected="true" onClick="toggleTab('create-tab')">Create Work
											Order</button>
									</li>
									<li class="nav-item" role="presentation">
										<button class="nav-link" id="orders-tab" data-bs-toggle="tab"
											data-bs-target="#orders" type="button" role="tab" aria-controls="orders"
											aria-selected="false" onClick="toggleTab('orders-tab')">Your Work
											Orders</button>
									</li>
								</ul>
								<div class="tab-content" id="workOrderTabsContent">
									...
									<div class="tab-pane fade show active" id="create" role="tabpanel"
										...
										<form method="post" action="main" enctype="multipart/form-data">
											<div class="mb-3">
												<label for="title" class="form-label">Title</label> <input name="title"
									...
									<div class="tab-pane fade" id="orders" role="tabpanel" aria-labelledby="orders-tab">
										...
										<c:forEach var="wo" items="${workOrders}">
											<div class="card mb-2">
												<div class="card-body">
													<h5 class="card-title">
														<c:out value="#${wo.number} - ${wo.title}" />
													</h5>
													<p class="card-text">
														<c:out value="${wo.description}" />
													</p>
													<span class="badge bg-secondary">
														<c:out value="#${wo.status}" />
													</span>
													<div class="text-muted mt-1" style="font-size: 0.85rem;">Created:
														<c:out value="${wo.createdAt}" />
													</div>
													<hr />
													<p>
														<strong>Solution:</strong>
														<c:out value="${wo.solution}" />
													</p>
													<p>
														<strong>Quote:</strong>
														<c:out value="$${wo.quote}" />
													</p>
													<p>
														<strong>Final Cost:</strong>
														<c:out value="$${wo.finalCost}" />
													</p>

													<!-- Attached Files Section -->
													<c:if test="${not empty wo.documents}">
														<hr />
														<p><strong>Attached Files:</strong></p>
														<div class="list-group list-group-flush">
															<c:forEach var="doc" items="${wo.documents}">
																<div
																	class="list-group-item d-flex justify-content-between align-items-center py-2">
																	<span>
																		<c:out value="${doc.documentName}" />
																	</span>
																	<button class="btn btn-sm btn-outline-primary"
																		onclick="viewFile(${doc.id})" title="View file">
													...
													<!-- Compliance Files Section (Admin only) -->
													<c:if test="${isAdmin and not empty wo.complianceDocuments}">
														<hr />
														<p><strong>Compliance Files:</strong></p>
														<div class="list-group list-group-flush">
															<c:forEach var="doc" items="${wo.complianceDocuments}">
																<div
																	class="list-group-item d-flex justify-content-between align-items-center py-2">
																	<span>
																		<c:out value="${doc.documentName}" />
																	...
																	<button class="btn btn-sm btn-outline-warning"
																		...
																		<i class="bi bi-shield-check"></i> View
																	</button>
																</div>
															</c:forEach>
														</div>
				...
				<script>
					...
					function viewFile(docId) {
						const url = 'file-viewer?docId=' + docId;
